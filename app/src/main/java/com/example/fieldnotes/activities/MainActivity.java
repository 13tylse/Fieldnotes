...
This class is the starting point for the entire application. This shows a list
of all saved Notebooks from the database. Let's break down some of this code:

An ugly listener for receiving touches from the user is notebookView.addOnItemTouchListener.
This calculates whether the user is touching on a notebook or scrolling through the list.
case MotionEvent.ACTION_DOWN is when the user touches the screen, we get that position and set a flag
to remind us that a click has happened. Case MotionEvent.ACTION_MOVE and case MotionEvent.ACTION_UP
are the two main cases that have vastly different logic. If ACTION_MOVE happens then we sit the
flag that the user has clicked to false, to not accidentally open up a notebook while scrolling.
But if ACTION_UP happens then we determine the notebook where the user clicked by getting the view
that was clicked and retrieving the unixTime from it.
*/
...
public class MainActivity extends AppCompatActivity {
    ...
    /**
     * Handles when the user chooses "Export" from the dropdown menu. Opens a scroll popup
     * that lists all the names of the <code>Notebooks</code>. This method simply handles
     * getting which <code>Notebook</code> the user selects, and then sends its
     * <code>unixTime</code> to the <code>sendToExportActivity</code> method to
     * send the user to the <code>ExportActivity</code>.
     */
    private void exportNotebook() {
        final AlertDialog optionDialog = new AlertDialog.Builder(this).create();
        optionDialog.setCancelable(true);
        optionDialog.setTitle("Choose Notebook to Export");
                    ...
                    case MotionEvent.ACTION_UP:
                        if (isOnClick) {
                            ...
                            if (layout != null) {
                                ...
                                sendToExportActivity(Long.parseLong(notebookUnix.getText().toString()));
        ...
        final NotebookViewAdapter exportAdapter = new NotebookViewAdapter(this);
        notebookView.setAdapter(exportAdapter);
        notebookView.setLayoutManager(new LinearLayoutManager(this));
        viewModel.getNotebooks().observe(this, new Observer<List<Notebook>>() {
            @Override
            public void onChanged(@Nullable final List<Notebook> notebooks) {
                // Update the cached copy of the words in the adapter.
                exportAdapter.setNotebooks(notebooks);
            }
        });

        optionDialog.setView(notebookView);
        optionDialog.setButton(AlertDialog.BUTTON_NEGATIVE, "Cancel", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
            }
    ...
    private void sendToExportActivity(long unixTime) {
