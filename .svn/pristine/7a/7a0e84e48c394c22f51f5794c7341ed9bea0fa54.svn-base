...
This was originally a class made exclusively for testing our picture displays before we got the
database up and running, but it did develop some useful functions we ended up using.

getLastPicture, getLastNPictures, and getLastNPicturePaths are not used, and were originally
test data. They're not too hard to understand, they just query the phone's pictures folder and
returns the last few pictures taken.

What this class is actually used for in our working code is reorienting pictures. There was an
issue where on some devices, portrait photos would be displayed rotated 90 degrees. The
method getOrientation simply looks at the filepath of a picture and returns its orientation
(which is kept as an integer in the Android system.)

The fixOrientation method takes a Bitmap and its orientation, and rotates the Bitmap according to
what its orientation is and its orientation should be. The code is pretty intuitive, considering
the ExifInterface class has constants for degrees of rotation.
 */
